...
- `precision=nothing`: accelerated arithmetic precision (`Float32` or `Float64`); an explicit precision requires `backend`. The default Float32 path uses bounded exact integer tail comparisons only on its validated input domain; Float64 p-values use floating-point comparisons. Summaries and retained draws remain backend-derived and approximate. `comparison=:cpu` replays the accelerated samples through the native CPU statistic and tolerance to replace only p-values. With Float64, it also uses ordinary CPU centering, so Geary scores and summaries can differ from the default shift-first path. `nothing` preserves the default CPU behavior when no backend is supplied.
